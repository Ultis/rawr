	...
	public static class ItemIcons
	{
		...
		public static void CacheAllIcons(Item[] items)
		{
			...
			foreach (Item item in items)
			{
				...
				localPath = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "images\\" + iconPath + ".jpg");
		...
		public static Image GetItemIcon(Item item) { return GetItemIcon(item, false); }
		public static Image GetItemIcon(Item item, bool small) { return GetItemIcon(item.IconPath, small); }
		public static Image GetItemIcon(string iconPath) { return GetItemIcon(iconPath, false); }
		public static Image GetItemIcon(string iconPath, bool small)
		{
			iconPath = iconPath.Replace(".png", "").Replace(".jpg", "").ToLower();
			if ((!small && !LargeIcons.Images.ContainsKey(iconPath)) || (small && !SmallIcons.Images.ContainsKey(iconPath)))
			{
				string localPath = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "images\\" + iconPath + ".jpg");
				...
				{
					MessageBox.Show("Rawr encountered an error while attempting to load a saved image, so is retrying. If you encounter this error multiple times, please ensure that Rawr is unzipped in a location that you have full file read/write access, such as your Desktop, or My Documents.");
					try
					{
		...
		private static void DownloadIcon(string iconPath, string localPath)
		{
			...
			try
			{
				...
				client.DownloadFile("http://armory.worldofwarcraft.com/images/icons/64x64/" + iconPath + ".jpg", localPath);
			}
			catch (Exception ex)
			{
				...
				{
					System.Windows.Forms.MessageBox.Show("Rawr encountered an error getting Image from Armory: " + localPath + ". Please copy and paste this into an e-mail to cnervig@hotmail.com. Thanks!\r\n\r\n\r\n" + ex.Message + "\r\n\r\n" + ex.StackTrace);
