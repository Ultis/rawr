...
namespace Rawr
{
	...
	public class Item : IComparable<Item>
	{
		...
		public string SlotString
		{
			...
			set 
            {
                _slot = (ItemSlot)Enum.Parse(typeof(ItemSlot), value, true);
                UpdateGemInformation();
            }
		}
		...
		public string TypeString
		{
			...
			set { _type = (ItemType)Enum.Parse(typeof(ItemType), value, true); }
		}
		[XmlIgnore]
		...
		public string SocketColor1String
		{
			...
			set { _socketColor1 = (ItemSlot)Enum.Parse(typeof(ItemSlot), value, true); }
		}
		[XmlIgnore]
		public string SocketColor2String
		{
			get { return _socketColor2.ToString(); }
            set { _socketColor2 = (ItemSlot)Enum.Parse(typeof(ItemSlot), value, true); }
		}
		[XmlIgnore]
		public string SocketColor3String
		{
			get { return _socketColor3.ToString(); }
            set { _socketColor3 = (ItemSlot)Enum.Parse(typeof(ItemSlot), value, true); }
		}
		[XmlIgnore]
	...
	public class ItemInstance : IComparable<ItemInstance>
	{
        ...
        public Stats AccumulateTotalStats(Character character, Stats unsafeStatsAccumulator)
		{
            if (cachedTotalStats != null && Item.LastChange <= cachedTime)
            {
                ...
                {
                    unsafeStatsAccumulator.Accumulate(cachedTotalStats);
                }
            ...
            {
                unsafeStatsAccumulator.Accumulate(Item.Stats);
                if (gem1) unsafeStatsAccumulator.Accumulate(Gem1.Stats);
                if (gem2) unsafeStatsAccumulator.Accumulate(Gem2.Stats);
                if (gem3) unsafeStatsAccumulator.Accumulate(Gem3.Stats);
                if (eligibleForSocketBonus) unsafeStatsAccumulator.Accumulate(Item.SocketBonus);
                ...
                if (eligibleForEnchant) unsafeStatsAccumulator.Accumulate(Enchant.Stats);
                return null;
            }
            else
            {
                Stats totalItemStats = new Stats();
                totalItemStats.Accumulate(Item.Stats);
                if (gem1) totalItemStats.Accumulate(Gem1.Stats);
                if (gem2) totalItemStats.Accumulate(Gem2.Stats);
                if (gem3) totalItemStats.Accumulate(Gem3.Stats);
                if (eligibleForSocketBonus) totalItemStats.Accumulate(Item.SocketBonus);
                bool eligibleForEnchant = false;
                if (Enchant.Slot == Item.ItemSlot.OneHand)
                {
                    eligibleForEnchant = (this.Slot == Item.ItemSlot.OneHand ||
                ...
                if (eligibleForEnchant) totalItemStats.Accumulate(Enchant.Stats);
                if (!volatileItem)
                {
