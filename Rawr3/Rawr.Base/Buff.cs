    ...
    public class Buff
    {
            ...
            get
            {
                if (Calculations.Instance == null || _cachedModel != Calculations.Instance.ToString() || _relevantBuffs == null)
                {
                    if (Calculations.Instance != null)
                    {
                        ...
                        _relevantBuffs = new List<Buff>(AllBuffs.Where<Buff>(delegate (Buff b)
                            {return Calculations.IsBuffRelevant(b); }));
                    }
                    else
        ...
        public static List<Buff> AllSetBonuses
        {
            get
            {
                ...
                {
                    _allSetBonuses = new List<Buff>(AllBuffs.Where<Buff>(delegate (Buff b) { return !string.IsNullOrEmpty(b.SetName); }));
                }
        ...
        public static List<Buff> RelevantSetBonuses
        {
            get
            {
                if (Calculations.Instance == null || _cachedModel != Calculations.Instance.ToString() || _relevantSetBonuses == null)
                {
                    ...
                    {
                        _cachedModel = Calculations.Instance.ToString();
                        _relevantSetBonuses = new List<Buff>(AllBuffs.Where<Buff>(delegate(Buff b) {
                            return Calculations.HasRelevantStats(b.GetTotalStats())
                                && !string.IsNullOrEmpty(b.SetName);}));
                    }
                    else
