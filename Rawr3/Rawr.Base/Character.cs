...
namespace Rawr //O O . .
{
    ...
    public class Character
    {
        ...
        {
            return _activeBuffs.Any<Buff>(delegate(Buff b) { return b.Name == buff; });
        }

        public bool ActiveBuffsConflictingBuffContains(string conflictingBuff)
        {
            return _activeBuffs.Any<Buff>(delegate(Buff b) { return b.ConflictingBuffs.Contains(conflictingBuff); });
        }
        ...
        {
            string anyGem = id + ".*.*.*";
            List<string> list = new List<string>(_availableItems.Where<string>(delegate(string s) { return s.StartsWith(id); }));
            if (list.Contains(gemId + ".*"))
            {
                return ItemAvailability.Available;
            }
            else if (list.Any<string>(delegate(string s) { return s.StartsWith(gemId); }))
            {
            ...
            else if (list.Any<string>(delegate(string s) { return s.StartsWith(anyGem); }))
            {
        ...
        //    {
        //        // all enabled toggle
        //        if (_availableItems.Contains(id) || _availableItems.Any<string>(delegate(string s) { return s.StartsWith(anyGem); }))
        //        {
        //            _availableItems.Remove(id);
        //            _availableItems = new List<string>(_availableItems.Where<string>(delegate(string s) { return !s.StartsWith(anyGem); }));
        //        }
        //        else
        //        {
		...
		public void RecalculateSetBonuses()
        {
            ...
            ActiveBuffs = new List<Buff>(ActiveBuffs.Where<Buff>(delegate(Buff b) { return string.IsNullOrEmpty(b.SetName); }));
        ...
        public void Save(string path)
        {
            ...
            _activeBuffsXml = new List<string>(_activeBuffs.Select<Buff, string>(delegate(Buff b) { return b.Name; }));
            //_activeBuffs.ConvertAll(buff => buff.Name);
        ...
        public static Character LoadFromXml(string xml)
        {
			...
			if (!string.IsNullOrEmpty(xml))
            {
				try
				{
                    ...
                    character._activeBuffs = new List<Buff>(character._activeBuffsXml
                        .Select<string, Buff>(delegate(string s) { return Buff.GetBuffByName(s); })
                        .Where<Buff>(delegate(Buff b) { return b != null; }));
