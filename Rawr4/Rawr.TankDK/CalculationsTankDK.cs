...
//			StatsDK sPaperDoll = stats.Clone();
            ...
            CombatTable2 ct = new CombatTable2(TDK.Char, stats, DPSCalcs, TDK.opts);
...
//            stats.Stamina = (float)Math.Floor(stats.Stamina);
			...
			float fChanceToGetCrit = 0.06f - stats.CritChanceReduction;
...
//                stats.Accumulate(newStats, uptime);
...
#endif
            rot.m_CT.m_CState.m_Stats = stats;
            rot.ResetRotation();
            rot.Solver();
            rot.ReportRotation();
            #endregion
			...
			float fBossAverageAttackSpeed = 0;
...
//                    fNewIncPhysDPS = GetParryHastedDPS(chanceTargetParry, fCharacterShotCount, fBossAverageAttackSpeed, fRotDuration, fPerShotPhysical);
			...
			float fBurstSwingCount = GetBurstTime(fAvoidanceTotal, fEffectiveHealth, 0);
