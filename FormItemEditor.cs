...
namespace Rawr
{
    public partial class FormItemEditor : Form
    {
        ...
        public ListViewItem SelectedItem
        {
            ...
            set
            {
                if (_selectedItem != null)
                {
                    ...
                    if (oldItem.Slot == Item.ItemSlot.Red || oldItem.Slot == Item.ItemSlot.Orange ||
                        oldItem.Slot == Item.ItemSlot.Yellow
                        || oldItem.Slot == Item.ItemSlot.Green || oldItem.Slot == Item.ItemSlot.Blue ||
                        oldItem.Slot == Item.ItemSlot.Purple
        ...
        private void Item_IdsChanged(object sender, EventArgs e)
        {
        ...
        public FormItemEditor(Character character)
        {
            ...
            FormClosing += new FormClosingEventHandler(FormItemEditor_FormClosing);
        }

        private void FormItemEditor_FormClosing(object sender, FormClosingEventArgs e)
        {
            textBoxName.Focus(); //Force data changes to be applied through databinding...
            textBoxIcon.Focus(); //databinding doesn't seem to set the new value until focus has left the control
        }
        ...
        private void comboBoxSocket_TextChanged(object sender, EventArgs e)
        {
            itemButtonGem1.CharacterSlot = comboBoxSocket1.Text == Item.ItemSlot.Meta.ToString()
                                               ? Character.CharacterSlot.Metas
                                               : Character.CharacterSlot.Gems;
            itemButtonGem2.CharacterSlot = comboBoxSocket2.Text == Item.ItemSlot.Meta.ToString()
                                               ? Character.CharacterSlot.Metas
                                               : Character.CharacterSlot.Gems;
            itemButtonGem3.CharacterSlot = comboBoxSocket3.Text == Item.ItemSlot.Meta.ToString()
                                               ? Character.CharacterSlot.Metas
                                               : Character.CharacterSlot.Gems;
        }
        ...
        private void LoadItems(Item[] items)
        {
            ...
            foreach (Item item in items)
            {
                if (string.IsNullOrEmpty(textBoxFilter.Text) ||
                    item.Name.ToLower().Contains(textBoxFilter.Text.ToLower()))
                {
                    string slot = item.Slot.ToString();
                    if (item.Slot == Item.ItemSlot.Red || item.Slot == Item.ItemSlot.Orange ||
                        item.Slot == Item.ItemSlot.Yellow
                        || item.Slot == Item.ItemSlot.Green || item.Slot == Item.ItemSlot.Blue ||
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    if (newItem == null)
                    {
                        if (
                            MessageBox.Show(
                                "Unable to load item " + form.Value.ToString() +
                                ". Would you like to create the item blank and type in the values yourself?",
                                "Item not found. Create Blank?", MessageBoxButtons.YesNo) == DialogResult.Yes)
                        {
                            newItem =
                                new Item("New Item", Quality.Epic, form.Value, "temp", Item.ItemSlot.Head, new Stats(),
                                         new Sockets(), 0, 0, 0);
                    ...
                    {
                        ListViewItem newLvi =
                            new ListViewItem(newItem.Name, 0,
                                             listViewItems.Groups["listViewGroup" + newItem.Slot.ToString()]);
                        newLvi.Tag = newItem;
                        newLvi.ImageKey = EnsureIconPath(newItem.IconPath);
                        string slot = newItem.Slot.ToString();
                        if (newItem.Slot == Item.ItemSlot.Red || newItem.Slot == Item.ItemSlot.Orange ||
                            newItem.Slot == Item.ItemSlot.Yellow
                            || newItem.Slot == Item.ItemSlot.Green || newItem.Slot == Item.ItemSlot.Blue ||
                            newItem.Slot == Item.ItemSlot.Purple
                            || newItem.Slot == Item.ItemSlot.Prismatic || newItem.Slot == Item.ItemSlot.Meta)
                            slot = "Gems";
        ...
        private void buttonDuplicate_Click(object sender, EventArgs e)
        {
            ...
            Item copy =
                new Item(item.Name, item.Quality, item.Id, item.IconPath, item.Slot,
                         new Stats(item.Stats.Armor, item.Stats.Health, item.Stats.Agility,
                                   item.Stats.Stamina, item.Stats.DodgeRating, item.Stats.DefenseRating,
                                   item.Stats.Resilience), new Sockets(item.Sockets.Color1,
                                                                       item.Sockets.Color2, item.Sockets.Color3,
                                                                       new Stats(item.Sockets.Stats.Armor,
                                                                                 item.Sockets.Stats.Health,
                                                                                 item.Sockets.Stats.Agility,
                                                                                 item.Sockets.Stats.Stamina,
                                                                                 item.Sockets.Stats.DodgeRating,
                                                                                 item.Sockets.Stats.DefenseRating,
                                                                                 item.Sockets.Stats.Resilience)),
                         0, 0, 0);
            _changingItemCache = true;
            ...
            ListViewItem newLvi =
                new ListViewItem(copy.Name, 0, listViewItems.Groups["listViewGroup" + copy.Slot.ToString()]);
            newLvi.Tag = copy;
            ...
            if (copy.Slot == Item.ItemSlot.Red || copy.Slot == Item.ItemSlot.Orange || copy.Slot == Item.ItemSlot.Yellow
                || copy.Slot == Item.ItemSlot.Green || copy.Slot == Item.ItemSlot.Blue ||
                copy.Slot == Item.ItemSlot.Purple
