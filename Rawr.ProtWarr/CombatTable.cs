...
namespace Rawr.ProtWarr {
    ...
    public abstract class CombatTable {
        ...
        public float AnyLand    { get { return (1.0f - (Miss + Dodge + Parry)); } }
    ...
    public class DefendTable : CombatTable {
        protected override void Calculate() {
            ...
            if (Character.OffHand != null && Character.OffHand.Type == ItemType.Shield) {
                ...
                Block = Math.Max(0f,Math.Min(1.0f - tableSize, tempVal));
                tableSize += Block;
            }
            // Critical Hit
            Critical = Math.Max(0f,Math.Min(1.0f - tableSize, Lookup.TargetCritChance(Character, Stats)));
            tableSize += Critical;
            // Normal Hit
    ...
    public static class Lookup {
        ...
        public static float TargetCritChance(Character character, Stats stats) {
            CalculationOptionsProtWarr calcOpts = character.CalculationOptions as CalculationOptionsProtWarr;
            return Math.Max(0.0f, ((5.0f + LevelModifier(character)) / 100.0f) - StatConversion.GetDRAvoidanceChance(character, stats, HitResult.Crit, calcOpts.TargetLevel));
        }
