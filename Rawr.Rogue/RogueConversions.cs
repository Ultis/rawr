namespace Rawr.Rogue
{
    public class RogueConversions
    {
        public static readonly float AgilityToAP = 1.0f;
        public static readonly float AgilityToArmor = 2.0f;
        public static readonly float AgilityToCrit = 1.0f/83.333f;
        public static readonly float AgilityToDodge = 1.0f/41.49f;
        public static readonly float CritRatingToCrit = 1.0f/45.91f;
        public static readonly float ExpertiseRatingToExpertise = 1.0f/8.2f;
        public static readonly float ExpertiseToDodgeParryReduction = 0.25f;
        public static readonly float HasteRatingToHaste = 1.0f/32.79f;
        public static readonly float HitRatingToHit = 1.0f/32.79f;
        public static readonly float HitRatingToSpellHit = 1.0f / 26.232f;
        public static readonly float ParryRatingToParry = 1.0f/23.6538461538462f;
        public static readonly float StaminaToHP = 10.0f;
        public static readonly float StrengthToAP = 1.0f;
        public static readonly float ArmorPenRatingToArmorReduction = 1f / 15.39529991f;
    }
}