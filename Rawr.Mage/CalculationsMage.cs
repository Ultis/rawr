...
namespace Rawr.Mage
{
    ...
    class CalculationsMage : CalculationsBase
    {
        ...
        public override CharacterCalculationsBase GetCharacterCalculations(Character character, Item additionalItem)
        {
            ...
            lp[0, 1] = -calculatedStats.ManaRegen; // TODO add JoW
            lp[1, 1] = 1;
            lp[lpRows, 1] = 0; // TODO add wand dps
            // evocation
            ...
            lp[0, 2] = -calculatedStats.ManaRegen5SR - 0.15f * characterStats.Mana / 2f; // TODO add evocation weapons
            lp[1, 2] = 1;
            lp[2, 2] = 1;
            lp[lpRows, 2] = 0;
            // mana pot
            ...
            lp[0, 4] = -calculatedStats.ManaRegen5SR + (-Math.Min(3, 1 + (int)((calculatedStats.FightDuration - 30f) / 120f)) * 2400f - ((calculatedStats.FightDuration >= 390) ? 1100f : 0f) - ((calculatedStats.FightDuration >= 510) ? 850 : 0)) / (calculatedStats.MaxManaGem * calculatedStats.ManaPotionTime);
            lp[1, 4] = 1;
            lp[4, 4] = 1;
            lp[14, 4] = 1;
            lp[lpRows, 4] = 0;
            ...
            do
            {
                ...
                for (i = 0; i <= rows; i++)
                {
                    if (i != r)
                    {
                        ...
                        {
                            a[i, j] = a[i, j] - a[r, j] * v;
                            if (a[i, j] < 0.00000000000001 && a[i, j] > -0.00000000000001) a[i, j] = 0; // compensate for floating point errors
                        }
                    }
                }
                t = XN[c];
                        ...
                        *aij = 1;
                        ...
                        for (i = 0, ai = a; i <= rows; i++, ai += (cols + 1))
                        {
                            if (i != r)
                            {
                                ...
                                {
                                    *aij -= a[r * (cols + 1) + j] * v;
                                    if (*aij < 0.00000000000001 && *aij > -0.00000000000001) *aij = 0; // compensate for floating point errors
                                }
                            }
                        }
                        t = XN[c];
