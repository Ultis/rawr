...
namespace Rawr.Retribution {
    public class CombatStats {
        ...
        public static float GetMissChance(float hit, int targetLevel)        { return (float)Math.Max(0f, StatConversion.WHITE_MISS_CHANCE_CAP[ targetLevel - 80] - hit); }
        public static float GetDodgeChance(float expertise, int targetLevel) { return (float)Math.Max(0f, StatConversion.WHITE_DODGE_CHANCE_CAP[targetLevel - 80] - StatConversion.GetDodgeParryReducFromExpertise(expertise)); }
        public static float GetResistChance(float spellHit, int targetLevel) { return (float)Math.Max(0f, ResistChance[targetLevel - 80] - spellHit); }
        public static float GetParryChance(float expertise, int targetLevel) { return (float)Math.Max(0f, StatConversion.WHITE_PARRY_CHANCE_CAP[targetLevel - 80] - StatConversion.GetDodgeParryReducFromExpertise(expertise)); }
        ...
        public float GetRangeAvoid() { return 1f - GetMissChance(Stats.PhysicalHit, _calcOpts.TargetLevel); }
        public float GetSpellAvoid() { return 1f - GetResistChance(Stats.SpellHit, _calcOpts.TargetLevel); }
