...
namespace Rawr.Retribution {
    ...
    public class CalculationsRetribution : CalculationsBase {
        ...
        public override ComparisonCalculationBase CreateNewComparisonCalculation() { return new ComparisonCalculationRetribution(); }
        public override CharacterCalculationsBase CreateNewCharacterCalculations() { return new CharacterCalculationsRetribution(); }
        ...
        public override CharacterCalculationsBase GetCharacterCalculations(Character character, Item additionalItem, bool referenceCalculation, bool significantChange, bool needsDisplayCalculations) {
            ...
            calc.ToParry      = CombatStats.GetParryChance( stats.Expertise,   calcOpts.TargetLevel);
            calc.ToResist     = CombatStats.GetResistChance(stats.SpellHit,    calcOpts.TargetLevel);
            calc.TimeBehind   = calcOpts.InFront;

            Rotation rot; if (calcOpts.SimulateRotation) { rot = new Simulator(combats); } else { rot = new EffectiveCooldown(combats); }
        ...
        public override Stats GetCharacterStats(Character character, Item additionalItem) { return GetCharacterStats(character, additionalItem, true); }
        public Stats GetCharacterStats(Character character, Item additionalItem, bool computeAverageStats) {
            ...
            if (character.Race == CharacterRace.Dwarf && character.MainHand != null && character.MainHand.Type == ItemType.TwoHandMace) {
                statsRace.Expertise += 5f;
            }
            if (character.Race == CharacterRace.Human && character.MainHand != null && (character.MainHand.Type == ItemType.TwoHandMace || character.MainHand.Type == ItemType.TwoHandSword)) {
        ...
        private void ConvertRatings(Stats stats, PaladinTalents talents, int targetLevel) {
            stats.Strength      *= (1f + stats.BonusStrengthMultiplier)
                                 * (1f + talents.DivineStrength         * 0.03f);
            stats.Intellect     *= (1f + stats.BonusIntellectMultiplier)
                                 * (1f + talents.DivineIntellect        * 0.03f);
            stats.AttackPower    = (stats.AttackPower + stats.Strength  * 2f)
                                 * (1f + stats.BonusAttackPowerMultiplier);
            stats.Agility       *= (1f + stats.BonusAgilityMultiplier);
            stats.Stamina       *= (1f + stats.BonusStaminaMultiplier)
                                 * (1f + talents.SacredDuty             * 0.04f)
                                 * (1f + talents.CombatExpertise        * 0.02f);
            ...
            stats.Expertise     += talents.CombatExpertise  * 2f
                                 + StatConversion.GetExpertiseFromRating(stats.ExpertiseRating, CharacterClass.Paladin);

            float talentCrit     = talents.CombatExpertise  * 0.02f
                                 + talents.Conviction       * 0.01f
                                 + talents.SanctityOfBattle * 0.01f;
            stats.PhysicalCrit   = stats.PhysicalCrit
                                 + StatConversion.GetCritFromRating(stats.CritRating, CharacterClass.Paladin)
            ...
            stats.SpellCrit      = stats.SpellCrit
                                 ...
                                 + talentCrit
                                 - (targetLevel == 83 ? 0.03f : 0f);
            // Constrain it between 0% and 100%
            stats.PhysicalCrit   = (float)Math.Max(0f, Math.Min(1f, stats.PhysicalCrit));
            stats.SpellCrit      = (float)Math.Max(0f, Math.Min(1f, stats.SpellCrit));

            stats.PhysicalHaste  = (1f + stats.PhysicalHaste)
                                 * (1f + StatConversion.GetHasteFromRating(stats.HasteRating, CharacterClass.Paladin))
                                 * (1f + stats.Bloodlust)
                                 - 1f; // return it to a simple percentage

            stats.SpellPower    += stats.Stamina * talents.TouchedByTheLight * 0.1f
                                 + stats.AttackPower * talents.SheathOfLight * 0.1f;
        }
