...
{
    public abstract class Skill : Ability<PaladinTalents>
    {
        public Skill(Character character, Stats stats, AbilityType abilityType, DamageType damageType) : base (character, stats, abilityType, damageType) {}
    ...
    {
        public Judgement(Character character, Stats stats) : base(character, stats, AbilityType.Range, DamageType.Holy) 
        {
            ...
            AbilityDamageMulitplier[Multiplier.Others] = (1f + PaladinConstants.TWO_H_SPEC);
            AbilityDamageMulitplier[Multiplier.Sets] = (1f + _stats.JudgementMultiplier);
    ...
    {
        public JudgementOfRighteousness(Character character, Stats stats) : base(character, stats) 
        {
    ...
    {
        public JudgementOfTruth(Character character, Stats stats, float averageStack) : base(character, stats)
        {
    ...
    {
        public Inquisition(Character character, Stats stats, int hp) : base(character, stats, AbilityType.Spell, DamageType.Holy) 
        { 
            HP = hp;
            Cooldown = Duration;
        }
        private int HP;
        public float Duration { get { return (HP + _stats.BonusRet_T11_P4_InqHP) * 4f * (1f + Talents.InquiryOfFaith * PaladinConstants.INQUIRY_OF_FAITH_INQ); } }
    }

    public class TemplarsVerdict : Skill
    {
        public TemplarsVerdict(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Physical) 
        {
            ...
            AbilityDamageMulitplier[Multiplier.Sets] = (1f + _stats.TemplarsVerdictMultiplier);
    ...
    {
        public CrusaderStrike(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Physical) 
        {
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.MeleeMH);
            CT.AbilityCritCorr = Talents.RuleOfLaw * PaladinConstants.RULE_OF_LAW +
                                 ...
                                 _stats.CrusaderStrikeCrit;
            AbilityDamageMulitplier[Multiplier.Talents] = (1f + PaladinConstants.CRUSADE * Talents.Crusade);
            AbilityDamageMulitplier[Multiplier.Sets] = (1f + _stats.CrusaderStrikeMultiplier);
            Cooldown = PaladinConstants.CS_COOLDOWN / (Talents.SanctityOfBattle > 0 ? (1f + _stats.PhysicalHaste) : 1f);
            AbilityDamage = AbilityHelper.WeaponDamage(_character, _stats.AttackPower, true) * PaladinConstants.CS_DMG_BONUS + _stats.CrusaderStrikeDamage;
        }
    }

    public class HandofLight : Skill
    {
        public HandofLight(Character character, Stats stats, float amountBefore) : base(character, stats, AbilityType.Spell, DamageType.NoDD) 
        {
    ...
    public class DivineStorm : Skill
    {
        public DivineStorm(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Physical) 
        {
            ...
            AbilityDamage = AbilityHelper.WeaponDamage(_character, _stats.AttackPower, true) * PaladinConstants.DS_DMG_BONUS + _stats.DivineStormDamage; 
        }
    ...
    {
        public HammerOfWrath(Character character, Stats stats) : base(character, stats, AbilityType.Range, DamageType.Holy) 
        {
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.Ranged);
            CT.AbilityCritCorr = Talents.SanctifiedWrath * PaladinConstants.SANCTIFIED_WRATH;
            AbilityDamageMulitplier[Multiplier.Sets] = (1f + _stats.HammerOfWrathMultiplier);
    ...
    {
        public HolyWrath(Character character, Stats stats) : base(character, stats, AbilityType.Spell, DamageType.Holy) 
        {
            CT = new BaseSpellCombatTable(Character.BossOptions, _stats, Attacktype.Spell);
            CT.AbilityCritCorr = (Character.BossOptions.MobType == (int)MOB_TYPES.DEMON || Character.BossOptions.MobType == (int)MOB_TYPES.UNDEAD) ? 1f : 0;
            Cooldown = PaladinConstants.HOW_COOLDOWN;
    ...
    {
        public Consecration(Character character, Stats stats) : base(character, stats, AbilityType.Spell, DamageType.Holy) 
        {
            CT = new BaseSpellCombatTable(Character.BossOptions, _stats, Attacktype.Spell);
            CT.CanCrit = false;
            Cooldown = PaladinConstants.CONS_COOLDOWN;
            AbilityDamage = (PaladinConstants.CONS_BASE_DMG +
                             PaladinConstants.CONS_COEFF_SP * (_stats.SpellPower + _stats.ConsecrationSpellPower) +
    ...
    {
        public SealOfCommand(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Holy) 
        {
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.MeleeMH);
            AbilityDamageMulitplier[Multiplier.Others] = (1f + PaladinConstants.TWO_H_SPEC);
    ...
    {
        public Seal(Character character, Stats stats, AbilityType abilityType) : base(character, stats, abilityType, DamageType.Holy)
        {
            AbilityDamageMulitplier[Multiplier.Talents] = (1f + PaladinConstants.SEALS_OF_THE_PURE * Talents.SealsOfThePure);
            AbilityDamageMulitplier[Multiplier.Sets] = (1f + _stats.SealMultiplier);
            AbilityDamageMulitplier[Multiplier.Others] = (1f + PaladinConstants.TWO_H_SPEC);
        }
    }
    ...
    {
        public SealOfRighteousness(Character character, Stats stats) : base(character, stats, AbilityType.Spell) 
        {
            CT = new BaseSpellCombatTable(Character.BossOptions, _stats, Attacktype.Spell);
            CT.CanCrit = false;
            CT.CanMiss = false;
    ...
    {
        public SealOfTruth(Character character, Stats stats) : base(character, stats, AbilityType.Melee) 
        {
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.MeleeMH);
            CT.CanMiss = false;
    ...
    {
        public SealOfTruthDoT(Character character, Stats stats, float averageStack) : base(character, stats, AbilityType.Melee, DamageType.Holy)
        {
            AverageStackSize = averageStack;
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.MeleeMH);
            CT.CanMiss = false;
    ...
    {
        public White(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Physical) 
        {
    ...
    {
        public NullSeal(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Holy) 
        {
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.MeleeMH);
            AbilityDamage = 0;
        }
    ...
    {
        public NullSealDoT(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Holy) 
        {
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.MeleeMH);
            AbilityDamage = 0;
        }
    ...
    {
        public NullJudgement(Character character, Stats stats) : base(character, stats, AbilityType.Melee, DamageType.Holy) 
        {
            CT = new BasePhysicalYellowCombatTable(Character.BossOptions, _stats, Attacktype.Ranged);
            AbilityDamage = 0;
        }
    ...
    {
        public MagicDamage(Character character, Stats stats, DamageType damageType) : base(character, stats, AbilityType.Spell, damageType)
        {
            CT = new BaseSpellCombatTable(Character.BossOptions, _stats, Attacktype.Spell);
